...
class ControllerManager:
	...
	data = JointState()
        while not rospy.is_shutdown(): self.control_loop(self)
	...
	self.position_1=int(self.joint_state.position[0])
      	self.position_2=int(self.joint_state.position[1])
        self.position_3= 3600-int(self.joint_state.position[2])
        DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_position(1, self.position_1)
        DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_position(2, self.position_2)
        DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_position(3, self.position_3)
    def speed_control(self):
        self.speed_1=int(self.joint_state.velocity[0])
        self.speed_2=int(self.joint_state.velocity[1])
        self.speed_3=int(self.joint_state.velocity[2])        
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_speed(1, self.speed_1)
        DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_speed(2, self.speed_2)
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_speed(3, self.speed_3)
	...
	enabled=1;
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_torque_enabled(1, enabled)
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_torque_enabled(2  enabled):
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_torque_enabled(3, enabled):
        """
	...
	disabled=0;
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_torque_enabled(1, disabled)
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_torque_enabled(2, disabled):
	DynamixelIO('/dev/ttyACM0',57600,readback_echo=False).set_torque_enabled(3, disabled):
